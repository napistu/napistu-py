...
def test_genodexito_mapping_operations():
    ...
    assert test_mappings.issubset(INTERCONVERTIBLE_GENIC_ONTOLOGIES), \
    ...
    if protein_test_mappings:  # Only test if we have protein ontologies
        ...
        assert isinstance(geno.stacked_mappings, pd.DataFrame), \
            "stack_mappings should create a DataFrame"
        assert not geno.stacked_mappings.empty, \
            "Stacked mappings should not be empty"
        assert set(geno.stacked_mappings['ontology'].unique()) == protein_test_mappings, \
