...
def download_gtex_rnaseq(target_uri: str, url: str = GTEX_RNASEQ_EXPRESSION_URL) -> None:
...
def load_and_clean_gtex_data(gtex_data_path: str) -> pd.DataFrame:
    ...
    gtex_expression_data[numeric_cols] = gtex_expression_data[numeric_cols].apply(pd.to_numeric, errors='coerce')
