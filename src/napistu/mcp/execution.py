...
def register_components(mcp, session_context=None, object_registry=None):
    ...
    async def get_registry_summary() -> Dict[str, Any]:
        ...
        return {
            ...
            "object_types": {name: type(obj).__name__ for name, obj in _session_objects.items()},
        }
        ...
        try:
            if object_name:
                ...
                if not hasattr(obj, function_name):
                    return {"error": f"Method '{function_name}' not found on object '{object_name}'"}
            ...
            if result is not None:
                ...
                if hasattr(result, "to_dict"):
                    ...
                    return {
                        ...
                        "result_preview": result.to_dict() if hasattr(result, "__len__") and len(result) < 10 else "Result too large to preview",
                    }
    ...
    async def search_paths(
        ...
        try:
            ...
            if hasattr(network, "find_paths"):
                # Direct method call
                paths = network.find_paths(source_node, target_node, max_depth=max_depth)
            elif hasattr(napistu.graph, "find_paths"):
                # Function call
                paths = napistu.graph.find_paths(network, source_node, target_node, max_depth=max_depth)
            else:
            ...
            if hasattr(paths, "to_dict"):
                return {
                    "success": True,
                    "result_name": result_name,
                    "paths_found": len(paths) if hasattr(paths, "__len__") else "unknown",
                    "result_preview": paths.to_dict() if hasattr(paths, "__len__") and len(paths) < 10 else "Result too large to preview",
                }
            else:
                return {
                    "success": True,
