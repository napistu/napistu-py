...
def read(resource_uri, url, output):
    ...
    async def run_read_resource():
        print(f"📖 Reading Resource: {resource_uri}", file=output if output.name != '<stdout>' else None)
        ...
        if content:
            ...
            print("❌ Could not read resource", file=output if output.name != '<stdout>' else None)
...
def compare(local_url, remote_url):
    ...
    async def run_comparison():
        ...
        if local_health and remote_health:
            ...
            all_components = set(local_components.keys()) | set(remote_components.keys())
            
            for component in sorted(all_components):
                local_status = local_components.get(component, {}).get('status', 'missing')
                remote_status = remote_components.get(component, {}).get('status', 'missing')
                ...
                print(f"  {icon} {component}: Local={local_status}, Remote={remote_status}")
        else:
