...
class Edgelist:
    ...
    def merge_edgelists(
        ...
        Automatically determines merge method based on standard_merge_by properties.
        Both edgelists must have matching standard_merge_by values.
        ...
        """
        # Convert other to Edgelist if needed
        ...
        # Check that standard_merge_by properties match
        self_merge_by = self.standard_merge_by
        other_merge_by = other_el.standard_merge_by

        if self_merge_by != other_merge_by:
            raise ValueError(
                f"Cannot merge edgelists with different merge_by conventions: "
                f"self uses '{self_merge_by}', other uses '{other_merge_by}'. "
                f"Both edgelists must use the same column convention (source/target or from/to)."
            )

        # Merge directly on the source/target columns (works for both name and index)
