...
def _filter_one_reactions_cofactors(
    ...
    if COFACTOR_DEFS.EXCEPT_ANY in cofactor_filter.keys():
        detected_exceptions = set(cofactor_filter[COFACTOR_DEFS.EXCEPT_ANY]).intersection(
            rsc_labels_set
        )
    ...
    if COFACTOR_DEFS.AS_SUBSTRATE in cofactor_filter.keys():
        substrates_set = set(
            one_rxns_species[one_rxns_species[SBML_DFS.STOICHIOMETRY] < 0][COFACTOR_DEFS.COFACTOR].tolist()
        )
        substrates_detected = set(cofactor_filter[COFACTOR_DEFS.AS_SUBSTRATE]).intersection(
            substrates_set
        )
    ...
    dropped_species = one_rxns_species[
        one_rxns_species[COFACTOR_DEFS.COFACTOR].isin(cofactor_filter[COFACTOR_DEFS.IF_ALL])
    ]

    return dropped_species.assign(filter_reason=filter_type)[COFACTOR_DEFS.FILTER_REASON]
...
def _find_cofactor_species(
    ...
    if not isinstance(sbml_dfs, SBML_dfs):
        raise TypeError(
            f"sbml_dfs was type {type(sbml_dfs)} and must be an SBML_dfs"
        )
    ...
    species_identifiers[COFACTOR_DEFS.COFACTOR] = species_identifiers[ONTOLOGIES.CHEBI].map(chebi_to_cofactor)
    
    # Get unique cofactor species (deduplicated by s_id)
    unique_cofactor_species = (
        species_identifiers
        .drop_duplicates(subset=[SBML_DFS.S_ID])
        .sort_values(SBML_DFS.S_NAME)
    )
    
    unique_cofactor_names = sorted(unique_cofactor_species[COFACTOR_DEFS.COFACTOR].unique())
...
def _map_cofactors_to_reaction_species(
    ...
    species_to_cofactor = cofactor_species.set_index(SBML_DFS.S_ID)[COFACTOR_DEFS.COFACTOR]
    ...
    cofactor_rscspecies = sbml_dfs.reaction_species.merge(
        cofactor_cspecies[COFACTOR_DEFS.COFACTOR], left_on=SBML_DFS.SC_ID, right_index=True, how="inner"
    )
...
class CofactorChebiIDs(BaseModel):
    ...
    @classmethod
    def validate_cofactor_chebi_structure(cls, v: Dict[str, List[int]]) -> Dict[str, List[int]]:
        """
        ...
        for cofactor_name, chebi_ids in v.items():
            ...
            if not isinstance(cofactor_name, str) or not cofactor_name.strip():
                raise ValueError(f"Cofactor name must be a non-empty string, got: {cofactor_name}")
            
            # Check ChEBI IDs list
            if not isinstance(chebi_ids, list):
                raise ValueError(f"ChEBI IDs for '{cofactor_name}' must be a list, got: {type(chebi_ids)}")
            
            if not chebi_ids:
                raise ValueError(f"ChEBI IDs list for '{cofactor_name}' cannot be empty")
            
            # Check individual ChEBI IDs
            for chebi_id in chebi_ids:
                if not isinstance(chebi_id, int):
                    raise ValueError(f"ChEBI ID must be an integer, got: {chebi_id} for cofactor '{cofactor_name}'")
                
                if chebi_id <= 0:
                    raise ValueError(f"ChEBI ID must be positive, got: {chebi_id} for cofactor '{cofactor_name}'")
                ...
                if chebi_id in all_chebi_ids:
                    ...
                    for existing_cofactor, existing_ids in v.items():
                        if existing_cofactor != cofactor_name and chebi_id in existing_ids:
                            duplicates.append(f"ChEBI ID {chebi_id} appears in both '{existing_cofactor}' and '{cofactor_name}'")
                            break
                else:
...
class CofactorFilterRule(BaseModel):
    ...
    except_any: List[str] = Field(default_factory=list, description="Cofactors that will override the filter")
    as_substrate: List[str] = Field(default_factory=list, description="Cofactors that must be present as substrates")
    ...
    @field_validator(COFACTOR_DEFS.IF_ALL, COFACTOR_DEFS.EXCEPT_ANY, COFACTOR_DEFS.AS_SUBSTRATE)
    @classmethod
    def validate_cofactor_names(cls, v: List[str]) -> List[str]:
        """Validate that all cofactor names are non-empty strings."""
        for name in v:
            if not isinstance(name, str) or not name.strip():
                raise ValueError(f"Cofactor name must be a non-empty string, got: {name}")
        return v
...
class CofactorSchema(BaseModel):
    ...
    @classmethod
    def validate_schema_not_empty(cls, v: Dict[str, CofactorFilterRule]) -> Dict[str, CofactorFilterRule]:
